...
from dither_js import dither_js, INDIGO
...
/* ---------- Mesh + Dither Canvas ---------- */
/* ---------- Fondo del título: Dither Canvas ----------
   El canvas ocupa toda la cabecera y el texto se dibuja encima. La legibilidad
   se garantiza con un velo blanco (::after) sobre el efecto, no bajando su
   opacidad: así el dither se ve nítido en los bordes y suave bajo el texto. */
.doc-header.mesh{position:relative;overflow:hidden;background:var(--canvas);
  border-radius:0 0 12px 12px;margin-bottom:6px}
.mesh-bg{position:absolute;inset:0;z-index:0;pointer-events:none}
/* La textura se disuelve hacia el borde derecho de la columna: sin corte seco. */
.doc-header.mesh .mesh-bg{-webkit-mask-image:linear-gradient(90deg,#000 0%,#000 52%,rgba(0,0,0,0) 100%);
  mask-image:linear-gradient(90deg,#000 0%,#000 52%,rgba(0,0,0,0) 100%)}
.mesh-bg::before{content:"";position:absolute;inset:-40px;
  background:
...
/* Velo: cubre el H1 lo justo para leer y se abre hacia la derecha y el pie,
   donde el dither se ve nítido en lugar de lavado. */
.doc-header.mesh::after{content:"";position:absolute;inset:0;z-index:3;pointer-events:none;
  background:
    linear-gradient(180deg, rgba(255,255,255,.91) 0%, rgba(255,255,255,.79) 30%,
                    rgba(255,255,255,.50) 62%, rgba(255,255,255,.14) 84%, var(--canvas) 100%),
    linear-gradient(90deg, rgba(255,255,255,.42) 0%, rgba(255,255,255,.16) 48%,
                    rgba(255,255,255,0) 100%)}
/* El contenido del header va por encima del velo. */
.doc-header.mesh > *:not(.mesh-bg):not(.mesh-hint){position:relative;z-index:4}
.doc-header.mesh .kicker,.doc-header.mesh h1,.doc-header.mesh .lead,
.doc-header.mesh .meta,.doc-header.mesh .stat-row{position:relative;z-index:4}
.doc-header.mesh .stat-pill{background:rgba(255,255,255,.82);backdrop-filter:blur(4px)}
.mesh-hint{position:absolute;right:18px;top:16px;z-index:5;font-size:10px;letter-spacing:.1px;
  text-transform:uppercase;color:var(--mute);background:rgba(255,255,255,.78);backdrop-filter:blur(6px);
...
.doc-header.mesh{padding:54px 0 44px;border-bottom:none}
...
@media (max-width:960px){
  ...
  .doc-header.mesh{padding:40px 0 34px}
  .doc-header.mesh::after{background:
    linear-gradient(180deg, rgba(255,255,255,.93) 0%, rgba(255,255,255,.82) 46%,
                    rgba(255,255,255,.62) 76%, var(--canvas) 100%)}
  .mesh-hint{top:12px;right:12px}
}
...
@media print{
  .sidebar,.menu-btn,.overlay,.side-search,.mesh-bg,.mesh-hint{display:none!important}
  .content{margin:0}
...
<div class="content">
  <div class="doc">
    ...
    <header class="doc-header mesh" id="resumen">
...
<script>{dither_js(INDIGO, side_lo=1.0)}</script>
